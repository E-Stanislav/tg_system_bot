...
# Отключаем "errexit" (выход при ошибке) — иначе при отсутствии
# необязательных утилит (docker, iproute2, timeout и др.) скрипт
# может завершиться раньше времени и не вывести JSON.
# Параметры:
#   -u  – ошибка при обращении к неинициализированной переменной
#   -o pipefail – статус пайплайна = статус последней завершившейся с ошибкой команды
# Флаг -e убираем, чтобы терпимо относиться к non-zero exit-коду отдельных команд.
set -uo pipefail
IFS=$'\n\t'
